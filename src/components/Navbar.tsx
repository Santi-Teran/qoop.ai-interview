...
const Navbar = () => {
  ...
  return (
    <nav className="flex justify-between items-center p-4 shadow-sm shadow-gray-600 relative">
      {/* Logo */}
      <h1 className="text-xl font-bold">Smart Habit Tracker</h1>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed top-0 right-0 w-3/4 h-full bg-light-background dark:bg-dark-background shadow-lg flex flex-col p-6 z-50"
          >
            {/* Cerrar Menú */}
            <button className="self-end mb-6 text-light-textPrimary dark:text-dark-textPrimary" onClick={toggleMenu}>
              <X size={28} />
            </button>
